...
def run():
    ...
    mse,s_mse = run_pyESN(inputfile,outputfile,parameter,drawflag)
...
def run_pyESN(inputfile,outputfile,parameter,drawflag=False):
    ...
    for key in inputdata.keys()[:n_input]:
        train_ctrl.append(inputdata[key][:traintest_cutoff])
        test_ctrl.append(inputdata[key][traintest_cutoff:])
    for key in inputdata.keys()[n_input:]:
        train_output.append(inputdata[key][:traintest_cutoff])
        train_output.append(inputdata[key][traintest_cutoff:])
    ...
    esn = ESN(**load_parameter(parameter))
